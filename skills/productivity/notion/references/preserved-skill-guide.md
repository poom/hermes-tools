...
Add those exports to your shell profile (or to `<home>/.hermes/.env`) so every session inherits them.
...
### Read page as Markdown (agent-friendly) (2)
...
### Query a database (data source) (2)
```bash
...
### Path C — mcporter / Notion MCP fallback

When `NOTION_API_KEY`/`ntn` are unavailable but the user hints at MCP (for example “mcporter notion-finn?”), check mcporter before declaring Notion inaccessible:

```bash
mcporter list --json
mcporter list notion-finn --schema --json
mcporter auth notion-finn          # if status is auth/401
mcporter call notion-finn.notion-fetch id='<notion page or database URL>'
mcporter call notion-finn.notion-search query='Unclaimed' query_type=internal \
  content_search_mode=workspace_search data_source_url='collection://...' \
  page_size:=25 max_highlight_length:=0
```

For Notion databases, first fetch the database URL to get the `<data-source url="collection://...">` ID and schema, then search/fetch records from that data source. For assignment boards, verify candidate records by fetching each page and reading the compact JSON row in the page text rather than trusting search result titles alone.
