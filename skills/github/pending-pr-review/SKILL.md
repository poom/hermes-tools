...
5. If the user asks about the last run outcome, inspect the latest `~/.hermes/cron/output/<job-id>/*.md` and relevant gateway error log lines; summarize compactly and preserve the queue distinction between raw pending items and “no unreviewed PRs remain.”
