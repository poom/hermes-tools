...
python3 ~/.hermes/skills/github/my-open-prs/scripts/my_open_prs.py --actions-json
...
Some older PR channels may not appear in `~/.hermes/my-open-prs/*.md` because they were created by the legacy `gh-pr-queue` flow. These commonly use lowercased names like `pr-<repo-name>-<pr-number>` (for example `pr-ewa-actions-425`) and have topics like `EWA-Services/<Repo> #<number> — managed by Hermes gh-pr-queue`. Current `my-open-prs` channels use `<repo>-pr-<number>`.
