...
- A git-backed config repo, e.g. `~/Projects/hermes-config/skills` — backup/source-of-truth for selected custom skills.
- A separate project/source repo, e.g. `~/Projects/agent-resources/skills/environment-harness` — true source of truth for one skill even if a copied version also exists in the config repo.
...
Example: `engineering-tools/environment-harness` may belong to `~/Projects/agent-resources/skills/environment-harness` even though a copy also exists under `~/Projects/hermes-config/skills/engineering-tools/environment-harness`.

If the user corrects this after repointing, restore only that symlink:

```text
$HERMES_HOME/skills/engineering-tools/environment-harness -> ~/Projects/agent-resources/skills/environment-harness
```
