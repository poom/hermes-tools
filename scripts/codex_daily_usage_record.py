...
from hermes_tools_common import hermes_home, which_executable

HOME = Path.home()
HERMES_HOME = hermes_home()
CODEX_SESSIONS = Path(os.environ.get("CODEX_SESSIONS_DIR", HOME / ".codex" / "sessions")).expanduser()
OUT_DIR = Path(os.environ.get("CODEX_USAGE_OUT_DIR", HERMES_HOME / "usage")).expanduser()


def safe_machine_id() -> str:
    raw = os.environ.get("CODEX_USAGE_MACHINE_ID") or socket.gethostname() or "unknown-machine"
    raw = raw.split(".", 1)[0]
    # Set CODEX_USAGE_MACHINE_ID for a stable cross-machine display name.
...
def find_bunx() -> str | None:
    for candidate in (
        which_executable("bunx", "BUNX"),
        str(HOME / ".bun" / "bin" / "bunx"),
    ):
...
def ccusage_one_day_report(day: str) -> str:
    """Return a compact ccusage daily summary for the local day, or a warning line."""
    bunx = find_bunx()
    if not bunx:
        return "ccusage 1-day: unavailable (bunx not found)"
    ...
    try:
        proc = subprocess.run(cmd, check=False, text=True, capture_output=True, timeout=120, env=env)
    except subprocess.TimeoutExpired:
        return "ccusage 1-day: unavailable (timed out after 120s)"
    except Exception as exc:
        return f"ccusage 1-day: unavailable ({exc})"

    if proc.returncode != 0:
        err = (proc.stderr or proc.stdout or "unknown error").strip().splitlines()
        detail = err[-1] if err else "unknown error"
        return f"ccusage 1-day: unavailable (exit {proc.returncode}: {detail[:200]})"

    try:
        data = json.loads(proc.stdout)
    except Exception as exc:
        return f"ccusage 1-day: unavailable (invalid JSON: {exc})"
    ...
    cache_creation = int(totals.get("cacheCreationTokens") or 0)
    cache_read = int(totals.get("cacheReadTokens") or 0)
    total_cost = float(totals.get("totalCost") or 0)
    daily_rows = data.get("daily") or []
    return (
        f"ccusage 1-day ({day}): {fmt(total_tokens)} total "
        f"({fmt(input_tokens)} input, {fmt(cache_creation)} cache creation, "
        f"{fmt(cache_read)} cache read, {fmt(output_tokens)} output), "
        f"${fmt(total_cost)} cost, {len(daily_rows)} daily row(s)."
    )
...
def main() -> int:
    ...
    ccusage_line = ccusage_one_day_report(today)
